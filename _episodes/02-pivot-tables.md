...
>
>Now, we see another ordering, but it starts at the lowest value, not the highest one. Since we’re interest in the most frequently occuring publishers, we will have to reorder the set:
>
>8. Click order: Ascending, and change the value to: Descending.
>
>Now we can begin inspecting the top publishers in this dataset:
>
...
After creating our first pivot table, we can now move on to making our first graph in Google Spreadsheets.

Exercise 2: Creating our first chart in Google Spreadsheets

1. First, select the top 10 publisher names and number of items with the mouse.
...
Now, we may have to wait a little bit, since we are generating a lot of summary data. Finally, you will see a large table with the authors as rows, and all different UiO libraries as columns.
...
2. Click on ‘Show:’ All items. Click ‘Clear’ to uncheck all library names. Now select ‘UiO HumSam-biblioteket’, ‘UiO Realfagsbiblioteket’, ‘UiO Medisinsk bibliotek Rikshospitalet’ and ‘UiO Juridisk bibliotek DB’. (Or your own favourite library!).
...
EasyBI.com suggests that Column charts should not have more than 7 categories.
